...
- `newNoteLocationMode` - controls where a new note is created when you click a link to a non-existing note. It has three modes:
  - `Default location` (the default) - the note is placed automatically under your **Default location for new notes**, and out-of-vault relative links (`[[../folder/note]]`) are refused with a notice - see [[02 Relative link fix]].
